...
def get_options():
    parser = argparse.ArgumentParser(
        description='Deploy Mozilla secure CloudTrail CloudFormation template '
                    'in every region')
    parser.add_argument(
        'bucketname',
    ...
    parser.add_argument(
        '--snsaccountid',
        help='AWS account ID of the secure CloudTrail storage account')
    parser.add_argument(
        '--snsregion', default=DEFAULT_SNS_REGION,
        help='AWS region of the secure CloudTrail storage account\'s SNS '
        'topic (default: %s)' % DEFAULT_SNS_REGION)
    parser.add_argument(
...
def deploy_stacks(bucketname,
    ...
    for region in boto.cloudtrail.regions():
        ...
        if len(current_cloudtrails) == 1:
            ...
            output['Existing CloudTrails'][
                region.name] = current_cloudtrails[0]
        ...
        cloudformation_template = json.dumps(cloudformation,
                                             ...
                                             indent=4,
                                             separators=(',', ': '))
        conn_cloudformation = boto.cloudformation.connect_to_region(
            region.name)
        ...
        if not dryrun:
            ...
            output['New CloudTrails'][region.name] = conn_cloudformation.create_stack(
                ...
                parameters=[("BucketName", bucketname),
                            ("SNSAccountId",
                             snsaccountid if snsaccountid is not None else ''),
                            ("SNSRegion", snsregion)],
                notification_arns=notification_arns)
            # notification_arns behavior changes between boto 2.25.0 (the
            # version present in Centos 7) and 2.26.0.
            ...
            logging.info('Dryrun preventing creation of CloudTrail in region'
                         ' %s' % region.name)
...
def report_status(data, snsregion, snsaccountid):
    if snsaccountid is not None:
        conn_sns = boto.sns.connect_to_region(snsregion)
        conn_sns.publish(target_arn=get_status_topic(snsregion, snsaccountid),
...
def main():
    ...
    # mozilla-cloudtrail-logs --snsaccountid 088944123687 --snsregion
    # us-west-2 --dryrun
    args = get_options()
    result = deploy_stacks(args.bucketname,
                           args.snsregion,
                           args.snsaccountid,
                           args.dryrun)
